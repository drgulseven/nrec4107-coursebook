...
where \(Price_i\) is product price, \(Volume_i\) is package volume, \(eta_0\) is the intercept, \(eta_1\) measures the relationship between volume and price, and \(u_i\) is the error term.

# Multiple Regression Model

\[
Price_i = eta_0 + eta_1 Volume_i + eta_2 Size_i + eta_3 Pieces_i + u_i
\]
...
\[
\ln(Y_i) = eta_0 + eta_1 X_i + u_i
\]

A one-unit increase in \(X\) is associated with an approximate \(100eta_1\)% change in \(Y\).

## Log-Log Model

\[
\ln(Y_i) = eta_0 + eta_1 \ln(X_i) + u_i
\]

The coefficient \(eta_1\) is an elasticity.
